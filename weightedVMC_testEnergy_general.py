...
batchsize=2000
save_dir=str(setting[8])
pathlib.Path(save_dir).mkdir(parents=True, exist_ok=True)
weights_filename=str(setting[9]) 
states_filename=str(setting[10])
print(f"save_dir: {save_dir}")
print(f"weights_filename: {weights_filename}")
print(f"states_filename: {states_filename}")
...
saved_states=np.load(states_filename)

print("saved_states shape=", saved_states.shape)
if saved_states.shape[0]>=5000: 
    states=(saved_states[test_iter,:parallel].reshape(-1,parallel,d))[-1]
else:
    states=(saved_states[-1,:parallel].reshape(-1,parallel,d))[-1]

print("test_states_start.shape=",states.shape)
cvstates=jcvcompute(states)
...
for iteration in range(iterations):
    ...
    Hloc_list.append(Hloc_raw.real/d-E0)
    print("%d  energy=%12.6e"%(iteration, energy))
    states_list.append(store_states)

    if iteration % 100 ==0:
        np.save(save_dir+'E_list%d_iter_%d.npy'%(myseed,test_iter) ,E_list)
        np.save(save_dir+'Hloc_list%d_iter_%d.npy'%(myseed,test_iter) ,Hloc_list)
        np.save(save_dir+'states%d_iter_%d.npy'%(myseed,test_iter) ,states_list)
